...
const STYLES = `
  ...
  .rd-search-clear { position:absolute; right:12px; top:50%; transform:translateY(-50%); background:#2a2d38; border:none; width:22px; height:22px; border-radius:50%; display:flex; align-items:center; justify-content:center; color:#9a96a0; font-size:14px; cursor:pointer; transition:background 0.2s, color 0.2s; }
  ...
  /* OVERLAY */
  .rd-overlay { position:fixed; inset:0; z-index:300; background:rgba(5,6,10,0.78); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:center; padding:20px; animation:rdFadeIn 0.2s ease; }
...
export default function ResignedDirectory() {
              ...
              {filtered.length > 0 ? filtered.map((emp, i) => (
                <tr key={emp.id} style={{ animationDelay:`${i*30}ms` }}>
                  ...
                  <td>
                    <div className="rd-actions">
                      <button className="rd-btn-view" title="View" onClick={() => setViewEmp(emp)}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                        View
                      </button>
                      <button className="rd-btn-delete" title="Delete" onClick={() => setDeleteTarget(emp)}>
                ...
                <tr><td colSpan={14} style={{padding:0}}>
                  <div className="rd-empty">
                    ...
                    <div className="rd-empty-title">{search ? `No results for "${search}"` : 'No resigned employees yet'}</div>
                    <div className="rd-empty-sub">{search ? 'Try a different keyword' : 'Resigned employees will appear here'}</div>
                  </div>
                </td></tr>
              )}
            </tbody>
            ...
            <div className="rd-card-section">
              ...
              <div className="rd-card-fields" style={{ marginBottom:14 }}>
                <div className="rd-card-field">
                  ...
                  <div className="rd-card-field-val">{viewEmp.cp_viber || '—'}</div>
                </div>
                <div className="rd-card-field">
                  <div className="rd-card-field-label">Official Email</div>
                  <div className="rd-card-field-val" style={{ fontSize:12 }}>{viewEmp.official_email || '—'}</div>
                </div>
                <div className="rd-card-field full">
                  <div className="rd-card-field-label">Home Address</div>
                  <div className="rd-card-field-val" style={{ whiteSpace:'normal' }}>{viewEmp.home_address || '—'}</div>
                </div>
                {viewEmp.reason && (
                  <div className="rd-card-field full">
        ...
        <div className="rd-overlay" onClick={() => setDeleteTarget(null)}>
          <div className="rd-confirm-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="rd-confirm-sub">
              You're about to permanently delete <strong style={{color:'#e8e4dc'}}>{deleteTarget.name}</strong>'s record.<br/>This cannot be undone.
            </div>
