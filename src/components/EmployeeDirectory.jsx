...
const STYLES = `
  ...
  .ed-search-clear { position:absolute; right:12px; top:50%; transform:translateY(-50%); background:#2a2d38; border:none; width:22px; height:22px; border-radius:50%; display:flex; align-items:center; justify-content:center; color:#9a96a0; font-size:14px; cursor:pointer; }
  ...
  .ed-btn-view, .ed-btn-edit, .ed-btn-resign, .ed-btn-delete { display:flex; align-items:center; justify-content:center; gap:4px; padding:6px 9px; border-radius:6px; font-family:'DM Sans',sans-serif; font-size:11px; font-weight:600; cursor:pointer; border:1px solid; transition:all 0.2s; white-space:nowrap; }
  ...
  .ed-overlay { position:fixed; top:0; left:0; right:0; bottom:0; z-index:9999; background:rgba(5,6,10,0.78); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:center; padding:20px; animation:edFadeIn 0.2s ease; }
  ...
  .ed-confirm-no:hover { border-color:rgba(255,255,255,0.15); color:#e8e4dc; }
`;
...
export default function EmployeeDirectory({ onNotify }) {
  ...
  const fetchEmployees = async () => {
    ...
    catch { notify('error', 'Failed to load employees'); }
  };
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      if (editingEmployee) { await axios.put(`/api/employees/${editingEmployee.id}`, formData); notify('success', 'Employee updated'); }
      else                 { await axios.post('/api/employees', formData); notify('success', 'Employee added'); }
      await fetchEmployees(); setShowModal(false);
    } catch { notify('error', 'Failed to save employee'); }
  ...
  const handleResign = async () => {
    ...
    } catch { notify('error', 'Failed to process resignation'); }
    finally { setSaving(false); }
  };
  ...
  const confirmDelete = async () => {
    try {
      ...
      await fetchEmployees(); setDeleteTarget(null);
      notify('success', 'Employee deleted');
    } catch { notify('error', 'Failed to delete employee'); }
  };
          ...
          <table className="ed-table">
            <thead>
              <tr>
                ...
                <th>SSS</th><th>PhilHealth</th><th>Pag-IBIG</th><th>TIN</th><th>CP / Viber</th><th>Email</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length > 0 ? filtered.map((emp, i) => (
                <tr key={emp.id} style={{ animationDelay:`${i*30}ms` }}>
                  <td><div className="ed-name-cell"><div className="ed-avatar">{getInitials(emp.name)}</div><span style={{fontWeight:600}}>{safeDisplay(emp.name)}</span></div></td>
                  ...
                  <td>
                    <div className="ed-actions">
                      <button className="ed-btn-view" onClick={() => setViewEmployee(emp)}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>View</button>
                      <button className="ed-btn-edit" onClick={() => openEditModal(emp)}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Edit</button>
                      <button className="ed-btn-resign" onClick={() => openResignModal(emp)}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>Resign</button>
                      <button className="ed-btn-delete" onClick={() => setDeleteTarget(emp)}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg>Delete</button>
                    </div>
                  </td>
                </tr>
              )) : (
                <tr><td colSpan={12} style={{padding:0}}><div className="ed-empty"><div className="ed-empty-icon">👥</div><div className="ed-empty-title">{search ? `No results for "${search}"` : 'No employees yet'}</div><div className="ed-empty-sub">{search ? 'Try a different keyword' : 'Click "Add Employee" to get started'}</div></div></td></tr>
              )}
            </tbody>
          </table>
        </div>
            ...
            <div className="ed-overlay" onClick={() => setViewEmployee(null)}>
              <div className="ed-view-card" onClick={e => e.stopPropagation()}>
                <div className="ed-card-top">
                  ...
                  <div className="ed-card-sub">{safeDisplay(viewEmployee.position)}<span className={`ed-status-chip ed-status-${viewEmployee.employment_status}`}>{safeDisplay(viewEmployee.employment_status)}</span></div>
                </div>
                <div className="ed-card-section">
                  ...
                  <div className="ed-card-fields" style={{marginBottom:14}}>
                    <div className="ed-card-field"><div className="ed-card-field-label">CP / Viber</div><div className="ed-card-field-val accent">{safeDisplay(viewEmployee.cp_viber)}</div></div>
                    <div className="ed-card-field"><div className="ed-card-field-label">Email</div><div className="ed-card-field-val" style={{fontSize:12}}>{safeDisplay(viewEmployee.official_email)}</div></div>
                    <div className="ed-card-field full"><div className="ed-card-field-label">Home Address</div><div className="ed-card-field-val" style={{whiteSpace:'normal'}}>{safeDisplay(viewEmployee.home_address)}</div></div>
                  </div>
                </div>
                <div className="ed-card-actions">
                  <button className="ed-btn-edit" style={{flex:1,justifyContent:'center',padding:'10px'}} onClick={() => openEditModal(viewEmployee)}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Edit</button>
                  <button className="ed-btn-resign" style={{flex:1,justifyContent:'center',padding:'10px'}} onClick={() => openResignModal(viewEmployee)}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>Resign</button>
                  <button className="ed-btn-delete" style={{flex:1,justifyContent:'center',padding:'10px'}} onClick={() => { setViewEmployee(null); setDeleteTarget(viewEmployee); }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg>Delete</button>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="ed-form-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="ed-form-section">
                  ...
                  <div className="ed-form-grid">
                    <div><label className="ed-field-label">Full Name <span className="ed-field-req">*</span></label><input className="ed-field-input" type="text" placeholder="e.g. Juan dela Cruz" value={formData.name} onChange={e => field('name', e.target.value)} /></div>
                    <div><label className="ed-field-label">Position</label><input className="ed-field-input" type="text" placeholder="e.g. Software Engineer" value={formData.position} onChange={e => field('position', e.target.value)} /></div>
                    <div><label className="ed-field-label">Diploma / Degree</label><input className="ed-field-input" type="text" placeholder="e.g. BS Computer Science" value={formData.diploma} onChange={e => field('diploma', e.target.value)} /></div>
                    <div><label className="ed-field-label">Employment Status</label><select className="ed-field-select" value={formData.employment_status} onChange={e => field('employment_status', e.target.value)}>{['Regular','Probationary','Contractual','Part-time'].map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                    <div><label className="ed-field-label">Date Started</label><input className="ed-field-input" type="date" value={formData.date_started} onChange={e => field('date_started', e.target.value)} /></div>
                    <div><label className="ed-field-label">Date Regularized</label><input className="ed-field-input" type="date" value={formData.date_regularized} onChange={e => field('date_regularized', e.target.value)} /></div>
                    <div><label className="ed-field-label">Salary (₱)</label><input className="ed-field-input" type="number" placeholder="0.00" value={formData.salary} onChange={e => field('salary', e.target.value)} /></div>
                  </div>
                  ...
                  <div className="ed-form-grid cols3">
                    {[
                      ...
                      <div key={f.key}><label className="ed-field-label">{f.label}</label><input className="ed-field-input" type="text" placeholder={f.ph} value={formData[f.key]} onChange={e => field(f.key, e.target.value)} /></div>
                    ))}
                  </div>
                  <div className="ed-form-section-label">Contact Info</div>
                  <div className="ed-form-grid">
                    <div><label className="ed-field-label">CP / Viber No.</label><input className="ed-field-input" type="text" placeholder="09XX-XXX-XXXX" value={formData.cp_viber} onChange={e => field('cp_viber', e.target.value)} /></div>
                    <div><label className="ed-field-label">Official Email</label><input className="ed-field-input" type="email" placeholder="email@company.com" value={formData.official_email} onChange={e => field('official_email', e.target.value)} /></div>
                  </div>
                  <div className="ed-form-grid single" style={{marginBottom:8}}>
                    <div><label className="ed-field-label">Home Address</label><input className="ed-field-input" type="text" placeholder="Full home address" value={formData.home_address} onChange={e => field('home_address', e.target.value)} /></div>
                  </div>
                </div>
              ...
              <div className="ed-resign-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="ed-resign-body">
                  <div className="ed-resign-who">
                    ...
                    <div><div className="ed-resign-who-name">{safeDisplay(selectedEmployee.name)}</div><div className="ed-resign-who-pos">{safeDisplay(selectedEmployee.position)}</div></div>
                  </div>
