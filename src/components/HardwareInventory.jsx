...
const STYLES = `
  ...
  /* OVERLAY */
  .hw-overlay { position:fixed; inset:0; z-index:300; background:rgba(5,6,10,0.78); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:center; padding:20px; animation:hwFadeIn 0.2s ease; }
...
export default function HardwareInventory() {
  ...
  const handleSubmit = async () => {
    if (!formData.hardware_type) return;
    setSaving(true);
    try {
      if (editingHardware) await axios.put(`/api/hardware/${editingHardware.id}`, formData);
      else                 await axios.post('/api/hardware', formData);
  ...
  const confirmDelete = async () => {
    try { await axios.delete(`/api/hardware/${deleteTarget.id}`); await fetchHardware(); setDeleteTarget(null); }
    catch (err) { console.error(err); }
  };
  ...
  return (
    <>
              ...
              {filtered.length > 0 ? filtered.map((item, i) => (
                <tr key={item.id} style={{ animationDelay:`${i*30}ms` }}>
                  ...
                  <td>
                    {item.user_fullname
                      ? <div className="hw-user-cell">
                          ...
                          <span style={{ fontSize:12 }}>{item.user_fullname}</span>
                        </div>
                  ...
                  <td>
                    <div className="hw-actions">
                      <button className="hw-btn-view" title="View" onClick={() => setViewItem(item)}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                        View
                      </button>
                      <button className="hw-btn-edit" title="Edit" onClick={() => openEditModal(item)}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                        Edit
                      </button>
                      <button className="hw-btn-delete" title="Delete" onClick={() => setDeleteTarget(item)}>
                ...
                <tr><td colSpan={10} style={{padding:0}}>
                  <div className="hw-empty">
                    ...
                    <div className="hw-empty-title">{search ? `No results for "${search}"` : 'No hardware records yet'}</div>
                    <div className="hw-empty-sub">{search ? 'Try a different keyword' : 'Click "Add Hardware" to get started'}</div>
                  </div>
                </td></tr>
              )}
            </tbody>
      ...
      {/* VIEW CARD - WITH LOCATION AT OFFICE */}
      {viewItem && (
        <div className="hw-overlay" onClick={() => setViewItem(null)}>
          <div className="hw-view-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="hw-card-section">
              ...
              {/* ✅ FIXED: Additional section with Location at Office */}
              {(viewItem.user_fullname || viewItem.location_at_office || viewItem.comment) && (
                <>
                  ...
                  <div className="hw-card-fields" style={{ marginBottom:20 }}>
                    ...
                    {/* ✅ LOCATION AT OFFICE - NOW DISPLAYED */}
                    ...
                    {viewItem.comment && (
                      <div className="hw-card-field" style={{gridColumn:'span 2'}}>
            ...
            <div className="hw-card-actions">
              ...
              <button className="hw-btn-delete" style={{flex:1,justifyContent:'center',padding:'10px'}} onClick={() => { setViewItem(null); setDeleteTarget(viewItem); }}>
            ...
            <div className="hw-form-section">
              ...
              <div className="hw-form-grid">
                <div>
                  ...
                  <select className="hw-field-select" value={formData.hardware_type} onChange={e => field('hardware_type', e.target.value)}>
                ...
                <div>
                  ...
                  <input className="hw-field-input" type="text" placeholder="e.g. Dell, Lenovo" value={formData.brand} onChange={e => field('brand', e.target.value)} />
                </div>
                <div>
                  <label className="hw-field-label">Windows Hostname</label>
                  <input className="hw-field-input" type="text" placeholder="e.g. PC-JUAN01" value={formData.windows_hostname} onChange={e => field('windows_hostname', e.target.value)} />
                </div>
                <div>
                  <label className="hw-field-label">Serial Number</label>
                  <input className="hw-field-input" type="text" placeholder="Serial / Dell Service Tag" value={formData.serial_number} onChange={e => field('serial_number', e.target.value)} />
                </div>
                <div>
                  <label className="hw-field-label">Windows Version</label>
                  <input className="hw-field-input" type="text" placeholder="e.g. Windows 11 Pro" value={formData.windows_version} onChange={e => field('windows_version', e.target.value)} />
                </div>
                <div>
                  <label className="hw-field-label">Windows Language</label>
                  <input className="hw-field-input" type="text" placeholder="e.g. English" value={formData.windows_language} onChange={e => field('windows_language', e.target.value)} />
                </div>
                <div>
                  <label className="hw-field-label">Keyboard Type</label>
                  <input className="hw-field-input" type="text" placeholder="e.g. US Layout" value={formData.keyboard_type} onChange={e => field('keyboard_type', e.target.value)} />
                </div>
                <div>
                  <label className="hw-field-label">Year of Purchase</label>
                  <input className="hw-field-input" type="text" placeholder="e.g. 2022" value={formData.year_of_purchase} onChange={e => field('year_of_purchase', e.target.value)} />
                </div>
              </div>
              ...
              <div className="hw-form-grid">
                {[
                  ...
                  <div key={f.key}>
                    ...
                    <select className="hw-field-select" value={formData[f.key]} onChange={e => field(f.key, e.target.value)}>
                ...
                <div>
                  ...
                  <input className="hw-field-input" type="text" placeholder="e.g. IT Room, Floor 2" value={formData.location_at_office} onChange={e => field('location_at_office', e.target.value)} />
                </div>
                <div>
                  <label className="hw-field-label">Assigned User</label>
                  <input className="hw-field-input" type="text" placeholder="e.g. Juan DELA CRUZ" value={formData.user_fullname} onChange={e => field('user_fullname', e.target.value)} />
                </div>
              </div>
              ...
              <div className="hw-form-grid single" style={{ marginBottom:8 }}>
                <div>
                  ...
                  <input className="hw-field-input" type="text" placeholder="Optional notes" value={formData.comment} onChange={e => field('comment', e.target.value)} />
                </div>
              </div>
            </div>
