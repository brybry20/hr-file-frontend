...
const STYLES = `
  ...
  .hw-wrap { font-family: 'DM Sans', sans-serif; color: #e8e4dc; position: relative; z-index: 10; }
  ...
  .hw-bg::before {
    ...
    background: radial-gradient(ellipse 60% 40% at 10% 20%, rgba(200,169,110,0.03) 0%, transparent 60%),
                radial-gradient(ellipse 50% 50% at 90% 80%, rgba(110,181,200,0.02) 0%, transparent 60%);
    animation: hwBgPulse 8s ease-in-out infinite;
  }
  .hw-bg::after {
    content: ''; position: absolute; inset: 0;
  ...
  @keyframes hwBgPulse { 0%,100%{opacity:0.5} 50%{opacity:1} }
  ...
  .hw-orb {
    ...
    animation: hwFloatOrb 20s ease-in-out infinite;
  }
  .hw-orb-1 { width:300px; height:300px; top:10%; right:-50px; }
  .hw-orb-2 { width:250px; height:250px; bottom:5%; left:-30px; animation-delay:5s;
    background: radial-gradient(circle at 70% 70%, rgba(90,120,200,0.08), transparent 70%); }
  @keyframes hwFloatOrb {
  ...
  .hw-header {
    ...
    animation: hwFadeDown 0.5s ease;
  }
  @keyframes hwFadeDown { from{opacity:0;transform:translateY(-20px)} to{opacity:1;transform:translateY(0)} }

  .hw-title-group { display: flex; align-items: center; gap: 16px; }
  .hw-title-icon {
    width: 48px; height: 48px;
    background: rgba(200,169,110,0.12); border: 1px solid rgba(200,169,110,0.3);
    border-radius: 12px; display: flex; align-items: center; justify-content: center;
    font-size: 22px; animation: hwIconPulse 3s ease-in-out infinite;
  }
  ...
  .hw-title { font-family:'DM Serif Display',serif; font-size:28px; color:#e8e4dc; letter-spacing:-0.5px; }
  .hw-count {
    font-size:11px; font-weight:600; letter-spacing:1.5px; text-transform:uppercase;
    color:#c8a96e; background:rgba(200,169,110,0.1); border:1px solid rgba(200,169,110,0.2);
    padding:4px 12px; border-radius:20px;
  }

  .hw-add-btn {
    display:flex; align-items:center; gap:8px;
    padding:12px 24px; background:#c8a96e; color:#0b0d12;
  ...
  .hw-add-btn:hover { background:#d4b87a; box-shadow:0 6px 20px rgba(200,169,110,0.4); transform:translateY(-2px); }
  .hw-add-btn:active { transform:translateY(0); }
  ...
  .hw-search {
    ...
    outline: none; transition: all 0.2s; caret-color: #c8a96e;
  }
  .hw-search::placeholder { color: #5a5a6a; }
  .hw-search:focus { border-color: rgba(200,169,110,0.5); box-shadow: 0 0 0 3px rgba(200,169,110,0.15); transform:translateY(-2px); }
  ...
  .hw-search-clear:hover { background: #3a3d4a; color: #e8e4dc; transform: translateY(-50%) scale(1.1); }

  .hw-result-info { font-size: 13px; color: #5a5a6a; margin-bottom: 16px; padding-left: 4px; }
  ...
  .hw-table-container {
    ...
    animation: hwSlideUp 0.5s ease 0.3s both;
  }
  @keyframes hwSlideUp { from{opacity:0;transform:translateY(30px)} to{opacity:1;transform:translateY(0)} }
  ...
  .hw-table tbody tr:hover { background:rgba(200,169,110,0.05); }
  .hw-table td { padding:16px 20px; font-size:14px; color:#e8e4dc; vertical-align:middle; }
  ...
  .hw-hostname {
    font-family:monospace; font-size:13px; color:#9a96a0; letter-spacing:0.5px;
  }
  ...
  .hw-avatar {
    ...
    display:flex; align-items:center; justify-content:center;
    font-size:12px; font-weight:700; color:#c8a96e; flex-shrink:0; transition:all 0.2s;
  }
  .hw-table tbody tr:hover .hw-avatar { background:rgba(200,169,110,0.2); transform:scale(1.05); }

  .hw-bool-true {
    display:inline-block; padding:4px 12px; border-radius:6px;
    background:rgba(90,206,168,0.12); border:1px solid rgba(90,206,168,0.2);
    font-size:12px; font-weight:600; color:#5acea8;
  }
  .hw-bool-false {
    display:inline-block; padding:4px 12px; border-radius:6px;
  ...
  .hw-muted { color:#5a5a6a; font-size:13px; }

  .hw-actions { display:flex; gap:6px; align-items:center; }
  .hw-btn-view, .hw-btn-edit, .hw-btn-delete {
  ...
  .hw-btn-view:hover   { background:rgba(200,169,110,0.2); border-color:rgba(200,169,110,0.4); transform:translateY(-2px); }
  .hw-btn-edit   { background:rgba(110,181,200,0.1); border-color:rgba(110,181,200,0.2); color:#6eb5c8; }
  .hw-btn-edit:hover   { background:rgba(110,181,200,0.2); border-color:rgba(110,181,200,0.4); transform:translateY(-2px); }
  .hw-btn-delete { background:rgba(224,90,90,0.1); border-color:rgba(224,90,90,0.2); color:#e05a5a; }
  .hw-btn-delete:hover { background:rgba(224,90,90,0.2); border-color:rgba(224,90,90,0.4); transform:translateY(-2px); }
  ...
  .hw-empty {
    ...
    animation:hwFadeScale 0.3s ease;
  }
  @keyframes hwFadeScale { from{opacity:0;transform:scale(0.95)} to{opacity:1;transform:scale(1)} }
  .hw-empty-icon { font-size:48px; opacity:0.5; animation:hwFloat 3s ease-in-out infinite; }
  @keyframes hwFloat { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-10px)} }
  .hw-empty-title { font-size:18px; color:#9a96a0; }
  .hw-empty-sub   { font-size:14px; color:#5a5a6a; }
  ...
  .hw-overlay {
    ...
    display: flex; align-items: center; justify-content: center;
    padding: 20px; animation: hwFadeIn 0.2s ease;
  }
  ...
  .hw-view-card {
    ...
    box-shadow: 0 40px 100px rgba(0,0,0,0.7), 0 0 0 1px rgba(200,169,110,0.1);
    animation: hwScaleIn 0.25s cubic-bezier(0.16,1,0.3,1); overflow: hidden;
  ...
  .hw-card-close:hover { background: rgba(255,255,255,0.1); color: #e8e4dc; transform: rotate(90deg); }
  .hw-card-avatar-lg {
    width: 64px; height: 64px; border-radius: 16px;
    background: rgba(200,169,110,0.15); border: 1px solid rgba(200,169,110,0.3);
    display: flex; align-items: center; justify-content: center;
    font-size: 26px; font-weight: 700; color: #c8a96e; margin-bottom: 16px;
    animation: hwGlow 3s ease-in-out infinite;
  }
  @keyframes hwGlow {
    0%,100%{ box-shadow:0 0 10px rgba(200,169,110,0.3); }
    50%    { box-shadow:0 0 25px rgba(200,169,110,0.6); }
  }
  .hw-card-name { font-family:'DM Serif Display',serif; font-size:22px; color:#e8e4dc; margin-bottom:8px; }
  .hw-card-sub  { font-size:13px; color:#c8a96e; font-weight:500; }

  .hw-card-section { padding:14px 28px 0; }
  .hw-card-section-title {
    font-size:11px; font-weight:700; letter-spacing:1.5px; text-transform:uppercase;
    color:#5a5a6a; margin-bottom:12px; padding-bottom:8px; border-bottom:1px solid rgba(255,255,255,0.05);
  }
  .hw-card-fields { display:grid; grid-template-columns:1fr 1fr; gap:0; margin-bottom:14px; }
  .hw-card-field {
  ...
  .hw-card-field.full { grid-column:span 2; padding-right:0; border-left:none; }
  .hw-card-field-label { font-size:10px; font-weight:600; letter-spacing:1px; text-transform:uppercase; color:#5a5a6a; margin-bottom:4px; }
  .hw-card-field-val { font-size:14px; font-weight:500; color:#e8e4dc; word-break:break-word; }
  .hw-card-field-val.accent { color:#c8a96e; }
  .hw-card-field-val.mono   { font-family:monospace; font-size:13px; color:#9a96a0; }
  ...
  .hw-form-title { font-family:'DM Serif Display',serif; font-size:22px; color:#e8e4dc; display:flex; align-items:center; gap:10px; }
  .hw-form-section { padding: 20px 28px 0; }
  .hw-form-section-label {
    font-size:11px; font-weight:700; letter-spacing:1.5px; text-transform:uppercase;
  ...
  .hw-field-input:focus, .hw-field-select:focus {
    border-color: rgba(200,169,110,0.5); box-shadow: 0 0 0 3px rgba(200,169,110,0.15); transform:translateY(-2px);
  }
  .hw-field-select {
  ...
  .hw-foot-save {
    padding: 12px 28px; background: #c8a96e; color: #0b0d12; border: none; border-radius: 10px;
    font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 700;
  ...
  .hw-foot-save:hover::before { transform: translateX(100%); }
  .hw-foot-save:hover { background: #d4b87a; box-shadow: 0 6px 20px rgba(200,169,110,0.4); transform: translateY(-2px); }
  .hw-foot-save:active { transform: translateY(0); }
  ...
  .hw-foot-cancel:hover { border-color: rgba(255,255,255,0.15); color: #e8e4dc; background: rgba(255,255,255,0.04); transform:translateY(-2px); }
  ...
  .hw-confirm-modal {
    ...
    animation: hwScaleIn 0.2s cubic-bezier(0.16,1,0.3,1); text-align: center;
    margin: 0 20px;
  }
  .hw-confirm-icon {
    ...
    display: flex; align-items: center; justify-content: center;
    font-size: 26px; margin: 0 auto 20px; animation: hwShake 0.3s ease;
  }
  @keyframes hwShake {
    0%,100%{transform:translateX(0)} 20%,60%{transform:translateX(-5px)} 40%,80%{transform:translateX(5px)}
  }
  .hw-confirm-title { font-family:'DM Serif Display',serif; font-size:22px; color:#e8e4dc; margin-bottom:10px; }
  .hw-confirm-sub   { font-size:14px; color:#9a96a0; margin-bottom:28px; line-height:1.6; }
  .hw-confirm-btns  { display:flex; gap:12px; }
  .hw-confirm-del {
  ...
  .hw-confirm-del::before {
    content:''; position:absolute; inset:0;
    background:linear-gradient(120deg, transparent 30%, rgba(255,255,255,0.2) 50%, transparent 70%);
    transform:translateX(-100%); transition:transform 0.5s;
  }
  .hw-confirm-del:hover::before { transform:translateX(100%); }
  .hw-confirm-del:hover { background:#ea6a6a; box-shadow:0 6px 20px rgba(224,90,90,0.4); transform:translateY(-2px); }
  .hw-confirm-no {
    flex:1; padding:12px; background:transparent; border:1px solid rgba(255,255,255,0.08);
    border-radius:10px; color:#9a96a0;
    font-family:'DM Sans',sans-serif; font-size:14px; font-weight:500; cursor:pointer; transition:all 0.2s;
  }
  .hw-confirm-no:hover { border-color:rgba(255,255,255,0.15); color:#e8e4dc; background:rgba(255,255,255,0.04); transform:translateY(-2px); }
`;
...
export default function HardwareInventory({ onNotify }) {
  ...
  const fetchHardware = async () => {
    try {
      ...
      notify('info', `Loaded ${res.data.length} hardware records`);
    } catch {
      notify('error', 'Failed to load hardware records');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingHardware) {
        await axios.put(`/api/hardware/${editingHardware.id}`, formData);
        notify('success', 'Hardware record updated successfully');
      } else {
        await axios.post('/api/hardware', formData);
        notify('success', 'Hardware record added successfully');
      }
      await fetchHardware();
      setShowModal(false);
    } catch {
      notify('error', 'Failed to save hardware record');
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmDelete = async () => {
    try {
      ...
      notify('success', 'Hardware record deleted successfully');
    } catch {
      notify('error', 'Failed to delete hardware record');
    }
  };
      ...
      <div className="hw-wrap">
        ...
        <div className="hw-search-wrap">
          ...
          <input
            className="hw-search"
            type="text"
            placeholder="Search by type, brand, hostname, serial, or user…"
            value={search}
        ...
        {search && (
          <div className="hw-result-info">
            Showing <span>{filtered.length}</span> of {hardware.length} records
          </div>
        )}
            ...
            <tbody>
              {filtered.length > 0 ? filtered.map((item, i) => (
                <tr key={item.id} style={{ animationDelay: `${i * 30}ms` }}>
                  ...
                  <td><span style={{fontSize:13,color:'#9a96a0'}}>{item.windows_version || <span className="hw-muted">—</span>}</span></td>
                  <td>
                    {item.user_fullname ? (
                      <div className="hw-user-cell">
                        <div className="hw-avatar">{getInitials(item.user_fullname)}</div>
                        <span style={{fontSize:13}}>{item.user_fullname}</span>
                      </div>
                    ) : (
                      <span className="hw-muted">—</span>
                    )}
                  </td>
                  <td><span style={{color:'#9a96a0',fontSize:13}}>{item.year_of_purchase || '—'}</span></td>
                  <td>{boolBadge(item.computer_at_office)}</td>
                  <td>{boolBadge(item.clarilog_installed)}</td>
                  <td>
                    <div className="hw-actions">
                      <button className="hw-btn-view" onClick={() => setViewItem(item)}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
                        </svg>
                        View
                      </button>
                      <button className="hw-btn-edit" onClick={() => openEditModal(item)}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      ...
                      <button className="hw-btn-delete" onClick={() => setDeleteTarget(item)}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <td colSpan={10} style={{ padding: 0 }}>
                    <div className="hw-empty">
                      ...
                      <div className="hw-empty-title">
                        {search ? `No results for "${search}"` : 'No hardware records yet'}
                      </div>
                      <div className="hw-empty-sub">
                        {search ? 'Try a different keyword' : 'Click "Add Hardware" to get started'}
                      </div>
                    </div>
                  </td>
                </tr>
                  ...
                  {(viewItem.user_fullname || viewItem.location_at_office || viewItem.comment) && (
                    <>
                      ...
                      <div className="hw-card-fields" style={{marginBottom:20}}>
                        {viewItem.user_fullname && (
                          <div className="hw-card-field">
                            <div className="hw-card-field-label">Assigned User</div>
                            <div className="hw-card-field-val accent">{viewItem.user_fullname}</div>
                          </div>
                        )}
                        {viewItem.location_at_office && (
                          <div className="hw-card-field">
                            <div className="hw-card-field-label">Location at Office</div>
                            <div className="hw-card-field-val">{viewItem.location_at_office}</div>
                          </div>
                        )}
                        {viewItem.comment && (
                          <div className="hw-card-field full">
                            <div className="hw-card-field-label">Comment</div>
                            <div className="hw-card-field-val" style={{whiteSpace:'normal'}}>{viewItem.comment}</div>
                          </div>
                        )}
                      </div>
                    </>
              ...
              <div className="hw-form-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="hw-form-section">
                  ...
                  <div className="hw-form-grid">
                    <div>
                      <label className="hw-field-label">Hardware Type *</label>
                      <select
                        className="hw-field-select"
                        value={formData.hardware_type}
                        onChange={e => field('hardware_type', e.target.value)}
                        autoFocus
                      >
                    ...
                    <div>
                      <label className="hw-field-label">Brand</label>
                      <input
                    ...
                    <div>
                      <label className="hw-field-label">Windows Hostname</label>
                      <input
                        className="hw-field-input"
                        type="text"
                        placeholder="e.g. PC-JUAN01"
                        value={formData.windows_hostname}
                        onChange={e => field('windows_hostname', e.target.value)}
                      />
                    </div>
                    <div>
                      <label className="hw-field-label">Serial Number</label>
                      <input
                        className="hw-field-input"
                        type="text"
                        placeholder="Serial / Dell Service Tag"
                        value={formData.serial_number}
                        onChange={e => field('serial_number', e.target.value)}
                      />
                    </div>
                    <div>
                      <label className="hw-field-label">Windows Version</label>
                      <input
                        className="hw-field-input"
                        type="text"
                        placeholder="e.g. Windows 11 Pro"
                        value={formData.windows_version}
                        onChange={e => field('windows_version', e.target.value)}
                      />
                    </div>
                    <div>
                      <label className="hw-field-label">Windows Language</label>
                      <input
                        className="hw-field-input"
                        type="text"
                        placeholder="e.g. English"
                        value={formData.windows_language}
                        onChange={e => field('windows_language', e.target.value)}
                      />
                    </div>
                    <div>
                      <label className="hw-field-label">Keyboard Type</label>
                      <input
                        className="hw-field-input"
                        type="text"
                        placeholder="e.g. US Layout"
                        value={formData.keyboard_type}
                        onChange={e => field('keyboard_type', e.target.value)}
                      />
                    </div>
                    <div>
                      <label className="hw-field-label">Year of Purchase</label>
                      <input
                        className="hw-field-input"
                        type="text"
                        placeholder="e.g. 2022"
                        value={formData.year_of_purchase}
                        onChange={e => field('year_of_purchase', e.target.value)}
                      />
                    </div>
                  </div>
                  ...
                  <div className="hw-form-grid">
                    {[
                      ...
                      <div key={f.key}>
                        <label className="hw-field-label">{f.label}</label>
                        <select
                          className="hw-field-select"
                          value={formData[f.key]}
                          onChange={e => field(f.key, e.target.value)}
                        >
                          {BOOL_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
                        </select>
                      </div>
                    ))}
                    <div>
                      <label className="hw-field-label">Location at Office</label>
                      <input
                        className="hw-field-input"
                        type="text"
                        placeholder="e.g. IT Room, Floor 2"
                        value={formData.location_at_office}
                        onChange={e => field('location_at_office', e.target.value)}
                      />
                    </div>
                    <div>
                      <label className="hw-field-label">Assigned User</label>
                      <input
                        className="hw-field-input"
                        type="text"
                        placeholder="e.g. Juan DELA CRUZ"
                        value={formData.user_fullname}
                        onChange={e => field('user_fullname', e.target.value)}
                      />
                    </div>
                  </div>
                  ...
                  <div className="hw-form-grid single" style={{marginBottom:8}}>
                    <div>
                      <label className="hw-field-label">Comment</label>
                      <input
                        className="hw-field-input"
                        type="text"
                ...
                <div className="hw-form-foot">
                  ...
                  <button className="hw-foot-save" onClick={handleSubmit} disabled={saving}>
                    {saving ? 'Saving…' : editingHardware ? 'Save Changes' : 'Add Hardware'}
                  </button>
                </div>
              </div>
            </div>
          ...
          {deleteTarget && (
            <div className="hw-overlay" onClick={() => setDeleteTarget(null)}>
              <div className="hw-confirm-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="hw-confirm-sub">
                  You're about to delete <strong style={{color:'#e8e4dc'}}>{deleteTarget.hardware_type} — {deleteTarget.brand}</strong>.<br/>
