...
const STYLES = `
  ...
  .ph-title-icon {
    ...
    background:rgba(169,110,200,0.12); border:1px solid rgba(169,110,200,0.25);
  ...
  .ph-add-btn {
    ...
    background:#a96ec8; color:#0b0d12; border:none; border-radius:9px;
  ...
  .ph-search {
    ...
    outline:none; transition:border-color 0.2s, box-shadow 0.2s; caret-color:#a96ec8;
  }
  ...
  .ph-avatar {
    ...
    background:rgba(169,110,200,0.12); border:1px solid rgba(169,110,200,0.25);
    display:flex; align-items:center; justify-content:center;
    font-size:12px; font-weight:700; color:#a96ec8; flex-shrink:0;
  }
  .ph-unit-chip {
    display:inline-flex; align-items:center; gap:5px; padding:3px 10px;
    background:rgba(169,110,200,0.08); border:1px solid rgba(169,110,200,0.15);
    border-radius:5px; font-size:12.5px; color:#a96ec8;
  }
  .ph-serial-chip {
    font-family:monospace; font-size:12.5px; color:#9a96a0; letter-spacing:0.5px;
  }
  .ph-number-chip {
    display:inline-flex; align-items:center; gap:5px; padding:3px 10px;
  ...
  /* OVERLAY */
  .ph-overlay { position:fixed; inset:0; z-index:300; background:rgba(5,6,10,0.78); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:center; padding:20px; animation:phFadeIn 0.2s ease; }
...
export default function PhoneInventory() {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      if (editingPhone) await axios.put(`/api/phones/${editingPhone.id}`, formData);
      else              await axios.post('/api/phones', formData);
  ...
  const confirmDelete = async () => {
    try { await axios.delete(`/api/phones/${deleteTarget.id}`); await fetchPhones(); setDeleteTarget(null); }
    catch (err) { console.error(err); }
  };
              ...
              {filtered.length > 0 ? filtered.map((phone, i) => (
                <tr key={phone.id} style={{ animationDelay:`${i*35}ms` }}>
                  <td>
                    <div className="ph-name-cell">
                      ...
                      <span style={{ fontWeight:500 }}>{phone.name}</span>
                    </div>
                  </td>
                  ...
                  <td>
                    <div className="ph-actions">
                      <button className="ph-btn-view" title="View" onClick={() => setViewPhone(phone)}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                        View
                      </button>
                      <button className="ph-btn-edit" title="Edit" onClick={() => openEditModal(phone)}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                        Edit
                      </button>
                      <button className="ph-btn-delete" title="Delete" onClick={() => setDeleteTarget(phone)}>
                ...
                <tr><td colSpan={5} style={{padding:0}}>
                  <div className="ph-empty">
                    ...
                    <div className="ph-empty-title">{search ? `No results for "${search}"` : 'No phone records yet'}</div>
                    <div className="ph-empty-sub">{search ? 'Try a different keyword' : 'Click "Add Phone" to get started'}</div>
                  </div>
                </td></tr>
              )}
            </tbody>
        ...
        <div className="ph-overlay" onClick={() => setViewPhone(null)}>
          <div className="ph-view-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="ph-card-actions">
              <button className="ph-btn-edit" style={{flex:1,justifyContent:'center',padding:'10px'}} onClick={() => openEditModal(viewPhone)}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                Edit
              </button>
              <button className="ph-btn-delete" style={{flex:1,justifyContent:'center',padding:'10px'}} onClick={() => { setViewPhone(null); setDeleteTarget(viewPhone); }}>
            ...
            <div className="ph-form-body">
              {[
                ...
                <div key={f.key}>
                  ...
                  <input className="ph-field-input" type="text" placeholder={f.ph}
                    value={formData[f.key]} onChange={e => field(f.key, e.target.value)}
