...
const STYLES = `
  ...
  .ph-wrap { font-family: 'DM Sans', sans-serif; color: #e8e4dc; position: relative; z-index: 10; }
  ...
  .ph-bg::before {
    ...
    background: radial-gradient(ellipse 60% 40% at 10% 20%, rgba(169,110,200,0.03) 0%, transparent 60%),
                radial-gradient(ellipse 50% 50% at 90% 80%, rgba(200,169,110,0.02) 0%, transparent 60%);
    animation: phBgPulse 8s ease-in-out infinite;
  }
  .ph-bg::after {
    content: ''; position: absolute; inset: 0;
  ...
  @keyframes phBgPulse { 0%,100%{opacity:0.5} 50%{opacity:1} }

  /* Floating Orbs */
  .ph-orb {
    position: fixed; border-radius: 50%;
    background: radial-gradient(circle at 30% 30%, rgba(169,110,200,0.08), transparent 70%);
    filter: blur(40px); pointer-events: none; z-index: 0;
    animation: phFloatOrb 20s ease-in-out infinite;
  }
  .ph-orb-1 { width:300px; height:300px; top:10%; right:-50px; }
  .ph-orb-2 { width:250px; height:250px; bottom:5%; left:-30px; animation-delay:5s;
  ...
  .ph-header {
    ...
    animation: phFadeDown 0.5s ease;
  }
  @keyframes phFadeDown { from{opacity:0;transform:translateY(-20px)} to{opacity:1;transform:translateY(0)} }

  .ph-title-group { display: flex; align-items: center; gap: 16px; }
  .ph-title-icon {
    width: 48px; height: 48px;
    background: rgba(169,110,200,0.12); border: 1px solid rgba(169,110,200,0.3);
    border-radius: 12px; display: flex; align-items: center; justify-content: center;
    font-size: 22px; animation: phIconPulse 3s ease-in-out infinite;
  }
  @keyframes phIconPulse {
    0%  { box-shadow: 0 0 0 0 rgba(169,110,200,0.3); }
    50% { box-shadow: 0 0 20px 5px rgba(169,110,200,0.5); }
    100%{ box-shadow: 0 0 0 0 rgba(169,110,200,0.3); }
  }
  .ph-title { font-family:'DM Serif Display',serif; font-size:28px; color:#e8e4dc; letter-spacing:-0.5px; }
  .ph-count {
    font-size:11px; font-weight:600; letter-spacing:1.5px; text-transform:uppercase;
    color:#a96ec8; background:rgba(169,110,200,0.1); border:1px solid rgba(169,110,200,0.2);
    padding:4px 12px; border-radius:20px;
  }

  .ph-add-btn {
    display:flex; align-items:center; gap:8px;
    padding:12px 24px; background:#a96ec8; color:#0b0d12;
  ...
  .ph-add-btn:hover { background:#b97ed8; box-shadow:0 6px 20px rgba(169,110,200,0.4); transform:translateY(-2px); }
  ...
  .ph-search {
    ...
    outline: none; transition: all 0.2s; caret-color: #a96ec8;
  }
  .ph-search::placeholder { color: #5a5a6a; }
  .ph-search:focus { border-color: rgba(169,110,200,0.5); box-shadow: 0 0 0 3px rgba(169,110,200,0.15); transform:translateY(-2px); }
  ...
  .ph-search-clear:hover { background: #3a3d4a; color: #e8e4dc; transform: translateY(-50%) scale(1.1); }

  .ph-result-info { font-size: 13px; color: #5a5a6a; margin-bottom: 16px; padding-left: 4px; }
  ...
  .ph-table-container {
    ...
    animation: phSlideUp 0.5s ease 0.3s both;
  }
  ...
  .ph-table tbody tr:hover { background:rgba(169,110,200,0.05); }
  .ph-table td { padding:16px 20px; font-size:14px; color:#e8e4dc; vertical-align:middle; }

  .ph-name-cell { display:flex; align-items:center; gap:14px; }
  .ph-avatar {
    width:38px; height:38px; border-radius:10px;
    background:rgba(169,110,200,0.12); border:1px solid rgba(169,110,200,0.2);
    display:flex; align-items:center; justify-content:center;
    font-size:14px; font-weight:700; color:#a96ec8; flex-shrink:0; transition:all 0.2s;
  }
  .ph-table tbody tr:hover .ph-avatar { background:rgba(169,110,200,0.2); transform:scale(1.05); }
  .ph-name-text { font-weight:500; transition:color 0.2s; }
  .ph-table tbody tr:hover .ph-name-text { color:#a96ec8; }

  .ph-unit-chip {
    display:inline-flex; align-items:center; gap:8px;
    padding:6px 14px; background:rgba(169,110,200,0.08);
    border:1px solid rgba(169,110,200,0.15); border-radius:8px;
    font-size:13px; font-weight:500; color:#a96ec8;
  }
  .ph-serial-chip {
  ...
  .ph-btn-view, .ph-btn-edit, .ph-btn-delete {
  ...
  .ph-btn-view:hover   { background:rgba(169,110,200,0.2); border-color:rgba(169,110,200,0.4); transform:translateY(-2px); }
  .ph-btn-edit   { background:rgba(200,169,110,0.1); border-color:rgba(200,169,110,0.2); color:#c8a96e; }
  .ph-btn-edit:hover   { background:rgba(200,169,110,0.2); border-color:rgba(200,169,110,0.4); transform:translateY(-2px); }
  .ph-btn-delete { background:rgba(224,90,90,0.1); border-color:rgba(224,90,90,0.2); color:#e05a5a; }
  .ph-btn-delete:hover { background:rgba(224,90,90,0.2); border-color:rgba(224,90,90,0.4); transform:translateY(-2px); }
  ...
  .ph-empty {
    ...
    animation:phFadeScale 0.3s ease;
  }
  @keyframes phFadeScale { from{opacity:0;transform:scale(0.95)} to{opacity:1;transform:scale(1)} }
  .ph-empty-icon { font-size:48px; opacity:0.5; animation:phFloat 3s ease-in-out infinite; }
  @keyframes phFloat { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-10px)} }
  .ph-empty-title { font-size:18px; color:#9a96a0; }
  .ph-empty-sub   { font-size:14px; color:#5a5a6a; }
  ...
  .ph-overlay {
    ...
    display: flex; align-items: center; justify-content: center;
    padding: 20px; animation: phFadeIn 0.2s ease;
  }
  ...
  .ph-view-card {
    ...
    box-shadow: 0 40px 100px rgba(0,0,0,0.7), 0 0 0 1px rgba(169,110,200,0.1);
    animation: phScaleIn 0.25s cubic-bezier(0.16,1,0.3,1); overflow: hidden;
    margin: 0 20px;
  }
  .ph-card-top {
    padding: 32px 28px 24px;
    background: linear-gradient(135deg, rgba(169,110,200,0.08) 0%, transparent 60%);
  ...
  .ph-card-close:hover { background: rgba(255,255,255,0.1); color: #e8e4dc; transform: rotate(90deg); }
  .ph-card-avatar-lg {
    width: 64px; height: 64px; border-radius: 16px;
    background: rgba(169,110,200,0.15); border: 1px solid rgba(169,110,200,0.3);
    display: flex; align-items: center; justify-content: center;
    font-size: 26px; font-weight: 700; color: #a96ec8; margin-bottom: 16px;
    animation: phGlow 3s ease-in-out infinite;
  }
  @keyframes phGlow {
    0%,100%{ box-shadow:0 0 10px rgba(169,110,200,0.3); }
    50%    { box-shadow:0 0 25px rgba(169,110,200,0.6); }
  }
  .ph-card-name { font-family:'DM Serif Display',serif; font-size:24px; color:#e8e4dc; margin-bottom:8px; }
  .ph-card-sub  { font-size:13px; color:#a96ec8; font-weight:500; }

  .ph-card-body { padding:24px 28px; }
  .ph-card-field {
    padding:16px 0; border-bottom:1px solid rgba(255,255,255,0.05);
    display:flex; flex-direction:column; gap:6px;
  }
  .ph-card-field:last-child { border-bottom:none; }
  .ph-card-field-label { font-size:11px; font-weight:600; letter-spacing:1.2px; text-transform:uppercase; color:#5a5a6a; }
  .ph-card-field-val { font-size:16px; font-weight:500; color:#e8e4dc; }
  .ph-card-field-val.accent { color:#a96ec8; }
  .ph-card-field-val.gold   { color:#c8a96e; }
  .ph-card-field-val.mono   { font-family:monospace; font-size:13px; color:#9a96a0; }
  ...
  .ph-form-head {
    ...
    display: flex; align-items: center; justify-content: space-between;
    background: linear-gradient(135deg, rgba(169,110,200,0.05) 0%, transparent 60%);
  }
  .ph-form-title { font-family:'DM Serif Display',serif; font-size:22px; color:#e8e4dc; display:flex; align-items:center; gap:10px; }
  .ph-form-body  { padding: 28px; display: flex; flex-direction: column; gap: 20px; }
  ...
  .ph-field-req { color: #a96ec8; margin-left: 2px; }
  ...
  .ph-field-input:focus { border-color: rgba(169,110,200,0.5); box-shadow: 0 0 0 3px rgba(169,110,200,0.15); transform:translateY(-2px); }
  ...
  .ph-foot-save {
    padding: 12px 28px; background: #a96ec8; color: #0b0d12; border: none; border-radius: 10px;
    font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 700;
  ...
  .ph-foot-save:hover::before { transform: translateX(100%); }
  .ph-foot-save:hover { background: #b97ed8; box-shadow: 0 6px 20px rgba(169,110,200,0.4); transform: translateY(-2px); }
  .ph-foot-save:active { transform: translateY(0); }
  ...
  .ph-foot-cancel:hover { border-color: rgba(255,255,255,0.15); color: #e8e4dc; background: rgba(255,255,255,0.04); transform:translateY(-2px); }
  ...
  .ph-confirm-modal {
    ...
    animation: phScaleIn 0.2s cubic-bezier(0.16,1,0.3,1); text-align: center;
    margin: 0 20px;
  }
  .ph-confirm-icon {
    ...
    display: flex; align-items: center; justify-content: center;
    font-size: 26px; margin: 0 auto 20px; animation: phShake 0.3s ease;
  }
  @keyframes phShake {
    0%,100%{transform:translateX(0)} 20%,60%{transform:translateX(-5px)} 40%,80%{transform:translateX(5px)}
  }
  .ph-confirm-title { font-family:'DM Serif Display',serif; font-size:22px; color:#e8e4dc; margin-bottom:10px; }
  .ph-confirm-sub   { font-size:14px; color:#9a96a0; margin-bottom:28px; line-height:1.6; }
  .ph-confirm-btns  { display:flex; gap:12px; }
  .ph-confirm-del {
  ...
  .ph-confirm-del::before {
    content:''; position:absolute; inset:0;
    background:linear-gradient(120deg, transparent 30%, rgba(255,255,255,0.2) 50%, transparent 70%);
    transform:translateX(-100%); transition:transform 0.5s;
  }
  .ph-confirm-del:hover::before { transform:translateX(100%); }
  .ph-confirm-del:hover { background:#ea6a6a; box-shadow:0 6px 20px rgba(224,90,90,0.4); transform:translateY(-2px); }
  .ph-confirm-no {
    flex:1; padding:12px; background:transparent; border:1px solid rgba(255,255,255,0.08);
    border-radius:10px; color:#9a96a0;
    font-family:'DM Sans',sans-serif; font-size:14px; font-weight:500; cursor:pointer; transition:all 0.2s;
  }
  .ph-confirm-no:hover { border-color:rgba(255,255,255,0.15); color:#e8e4dc; background:rgba(255,255,255,0.04); transform:translateY(-2px); }
`;
...
export default function PhoneInventory({ onNotify }) {
  ...
  const fetchPhones = async () => {
    try {
      ...
      setPhones(res.data);
      notify('info', `Loaded ${res.data.length} phone records`);
    } catch {
      notify('error', 'Failed to load phone records');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.unit.trim()) return notify('error', 'Unit/Model is required');
    if (!formData.serial_number.trim()) return notify('error', 'Serial number is required');
    if (!formData.cellphone_number.trim()) return notify('error', 'Cellphone number is required');
    setSaving(true);
    try {
      if (editingPhone) {
        await axios.put(`/api/phones/${editingPhone.id}`, formData);
        notify('success', 'Phone record updated successfully');
      } else {
        await axios.post('/api/phones', formData);
        notify('success', 'Phone record added successfully');
      }
      await fetchPhones();
      setShowModal(false);
    } catch {
      notify('error', 'Failed to save phone record');
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmDelete = async () => {
    try {
      ...
      notify('success', 'Phone record deleted successfully');
    } catch {
      notify('error', 'Failed to delete phone record');
    }
  };
      ...
      <div className="ph-wrap">
        ...
        <div className="ph-search-wrap">
          ...
          <input
            className="ph-search"
            type="text"
            placeholder="Search by name, unit, serial, or number…"
            value={search}
        ...
        {search && (
          <div className="ph-result-info">
            Showing <span>{filtered.length}</span> of {phones.length} records
          </div>
        )}
                ...
                <tr key={phone.id} style={{ animationDelay: `${i * 35}ms` }}>
                  <td>
                    <div className="ph-name-cell">
                      <div className="ph-avatar">{getInitials(phone.name)}</div>
                      <span className="ph-name-text">{phone.name}</span>
                    </div>
                  </td>
                  ...
                  <td>
                    <div className="ph-actions">
                      <button className="ph-btn-view" onClick={() => setViewPhone(phone)}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
                        </svg>
                        View
                      </button>
                      <button className="ph-btn-edit" onClick={() => openEditModal(phone)}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      ...
                      <button className="ph-btn-delete" onClick={() => setDeleteTarget(phone)}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <td colSpan={5} style={{ padding: 0 }}>
                    <div className="ph-empty">
                      ...
                      <div className="ph-empty-title">
                        {search ? `No results for "${search}"` : 'No phone records yet'}
                      </div>
                      <div className="ph-empty-sub">
                        {search ? 'Try a different keyword' : 'Click "Add Phone" to get started'}
                      </div>
                    </div>
                  </td>
                </tr>
            ...
            <div className="ph-overlay" onClick={() => setViewPhone(null)}>
              <div className="ph-view-card" onClick={e => e.stopPropagation()}>
                ...
                <div className="ph-card-actions">
                  <button
                    className="ph-btn-edit"
                    style={{ flex: 1, justifyContent: 'center', padding: '12px' }}
                    onClick={() => openEditModal(viewPhone)}
                  >
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/>
                      <path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/>
                  ...
                  <button
                    ...
                    style={{ flex: 1, justifyContent: 'center', padding: '12px' }}
                    onClick={() => { setViewPhone(null); setDeleteTarget(viewPhone); }}
                  >
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <polyline points="3 6 5 6 21 6"/>
                      <path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/>
              ...
              <div className="ph-form-modal" onClick={e => e.stopPropagation()}>
                <div className="ph-form-head">
                  <div className="ph-form-title">
                    {editingPhone ? '✏️' : '📱'}
                    {editingPhone ? 'Edit Phone' : 'New Phone'}
                  </div>
                  ...
                  {[
                    ...
                    <div key={f.key}>
                      <label className="ph-field-label">
                        {f.label} {f.req && <span className="ph-field-req">*</span>}
                      </label>
                      <input
                ...
                <div className="ph-form-foot">
                  ...
                  <button className="ph-foot-save" onClick={handleSubmit} disabled={saving}>
                    {saving ? 'Saving…' : editingPhone ? 'Save Changes' : 'Add Phone'}
                  </button>
                </div>
              </div>
            </div>
          ...
          {deleteTarget && (
            <div className="ph-overlay" onClick={() => setDeleteTarget(null)}>
              <div className="ph-confirm-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="ph-confirm-sub">
                  You're about to delete <strong style={{color:'#e8e4dc'}}>{deleteTarget.name}</strong>'s phone record.<br/>
